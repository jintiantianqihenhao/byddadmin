<template>
  <div class="print">
          <div class="dyTitle">查询未生成打印图订单</div>
...
export default {
  ...
  methods: {
      ...
      getMyList(){
             var token = localStorage.getItem('token');
             var url =  this.HOST+'/bmcs/api/orders/no/commit/list?page='+this.current+'&size=10&startTime=&endTime=&orderSource=&orderStatus=&searchInfo=';
             this.$axios.get(url,
              {headers: {
                "token":token
              }})
